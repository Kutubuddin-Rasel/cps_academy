...
export default async function PublicCoursePage({ params }: PageProps<"/courses/[courseId]">) {
  ...
  if (errorMessage) return <section className="rounded-2xl border border-red-200 bg-white p-6"><h1 className="text-2xl font-semibold">Course unavailable</h1><p role="alert" className="mt-3 text-red-800">{errorMessage}</p></section>;
  if (!course) notFound();
