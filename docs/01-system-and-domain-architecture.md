...
The following is a **logical domain ER diagram**. Strapi generates additional physical relation/component tables in PostgreSQL, so this diagram should not be interpreted as a literal table-by-table database schema.
...
erDiagram
    ...
    COURSE ||--o{ LESSON_PROGRESS : scopes
    COURSE ||--o{ QUIZ_ATTEMPT : scopes

    LESSON ||--o{ LESSON_PROGRESS : completed_as

    QUIZ ||--o{ QUESTION : embeds
    QUESTION ||--o{ OPTION : embeds
...
`QUESTION` and `OPTION` above are logical nodes representing **Strapi components embedded inside Quiz**, not independently addressable top-level Content API resources.
